class AllMethods {
    ...
    useDisplayedKeyboard() {
        ...
        Array.from(keys).forEach(btn => {
            btn.addEventListener('click', this.printByClick.bind(this))
        })
    }
    ...
    printLetter(input) {
        ...
        if (Array.from(chosenWord).includes(input)) {
            Array.from(chosenWord).map((letter, index) => {
                if (letter === input) wordToGuess[index].value = input
            })
    ...
    displayLooserDiv() {
        ...
        showWord.innerText = '';
        showWord.innerText = `"${this.selectedWord}"`;
